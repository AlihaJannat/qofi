...
document.getElementById("layout-menu") && (isHorizontalLayout = document.getElementById("layout-menu").classList.contains("menu-horizontal")),
    function() {
        ...
        window.templateCustomizer ? (n && n.addEventListener("click", function() {
            window.Helpers.isLightStyle() ? window.templateCustomizer.setStyle("dark") : window.templateCustomizer.setStyle("light")
        }), window.Helpers.isLightStyle() ? (n && (n.querySelector("i").classList.add("bx-moon"), new bootstrap.Tooltip(n, {
            title: "Dark mode",
            fallbackPlacements: ["bottom"]
        })), o("light")) : (n && (n.querySelector("i").classList.add("bx-sun"), new bootstrap.Tooltip(n, {
            title: "Light mode",
            fallbackPlacements: ["bottom"]
        })), o("dark"))) : n.parentElement.remove(), window.onscroll = function() {
        ...
        let s = document.getElementsByClassName("dropdown-language");
        if (s.length) {
            var a = s[0].querySelectorAll(".dropdown-item");
            for (let e = 0; e < a.length; e++) a[e].addEventListener("click", function() {
                ...
                this.classList.add("selected"), s[0].querySelector(".dropdown-toggle .fi").className = t, i18next.changeLanguage(e, (e, t) => {
