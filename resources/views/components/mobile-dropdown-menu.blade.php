<div id="{{ $submenuId }}" class="z-10 hidden item-submenu bg-transparent divide-y divide-gray-100 rounded-lg m-3">
    <div class="w-full h-full bg-transparent flex flex-col justify-start items-center">
            ...
            @if (count($subCategories))
                <div class="flex-1 py-8 px-4 lg:px-16 flex justify-start items-start gap-8">
                    <div class="flex-1 flex flex-col justify-start items-start gap-4">
                        <div class="w-full text-black-5 text-xl font-semibold font-roboto leading-[21px]">
                            Sub Categories for {{ $label }}</div>
            ...
            @if (count($products))
                <div
                    ...
                    <div class="py-1 flex justify-center items-center gap-2">
                        <div href="{{route('product.index')}}?selectedCategories[0]={{$categoryId}}" class="!text-black-5 text-base font-normal font-roboto leading-[24px]">See
                            all
                        </div>
