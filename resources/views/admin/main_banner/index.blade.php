            ...
            <div class="card-header flex-column flex-md-row">
                ...
                <div class="dt-action-buttons text-end pt-3 pt-md-0">
                    <div class="dt-buttons btn-group flex-wrap">
                        <a href="{{ route('admin.main-banner.new') }}" class="btn btn-secondary create-new btn-primary" tabindex="0"
...
@section('script')
    <script>
        ...
        function updateCouponStatus(id, status, button) {
            $.ajax({
                url: "{{ route('admin.main-banner.status') }}",
                method: "GET",
                data: {
                    id: id,
                    status: status
        ...
        $(document).ready(function() {
            var table = $('#bannnerTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.main-banner.yajra') }}",
                columns: [
                    {
                        data: 'sort_order',
                        name: 'sort_order'
            ...
            deleteBanner = function(itSelf, id) {
                ...
                Swal.fire({
                    ...
                    .then((result) => {
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('admin.main-banner.delete') }}",
                                method: "DELETE",
                                data: {
                                    _token: "{{ csrf_token() }}",
                                    id: id
